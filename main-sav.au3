#pragma compile(FileDescription, ECU Anti-Virus Setup)
#pragma compile(ProductName, ECU Anti-Virus Setup)
#pragma compile(ProductVersion, 3.8)
#pragma compile(FileVersion, 3.8)
...
If FileExists($conffile) == 0 Then
   FileInstall("config.ini", @TempDir & "\SAVconfig.ini")
EndIf
...
DirRemove(@HomeDrive & "\SAVLogs", 1)
DirRemove(@TempDir & "\SAVInstall")
...
DirCreate(@TempDir & "\SAVInstall")
DirCreate(@HomeDrive & "\SAVLogs")
FileInstall("createrestore.vbs", @TempDir & "\SAVInstall\createrestore.vbs")
FileInstall("createrestore.bat", @TempDir & "\SAVInstall\createrestore.bat")

;Creates a base log file. Might deprecate this since the log files for SAV are broken.
Global $logname = @HomeDrive & "\SAVLogs\Main-log.txt"
Global $SAVlogname = @HomeDrive & "\SAVLogs\SAV-install-log.txt"
_FileCreate($logname)
FileWrite($logname, "The Anti-Virus Installer log file stores the installer logs and command outputs." & @CRLF)

;Get SAV Version
Global $curSAVver = "12.1.7004.6500"

RunWait(@ComSpec & " /c " & "wmic product where name=""Symantec Endpoint Protection"" get version > " & @TempDir & "\SAVInstall\sav_ver.txt", "", @SW_HIDE)
FileOpen(@TempDir & "\SAVInstall\sav_ver.txt")
Global $installedSAVver = FileReadLine(@TempDir & "\SAVInstall\sav_ver.txt", 2)
$installedSAVver = StringStripWS($installedSAVver, 8)
FileClose(@TempDir & "\SAVInstall\sav_ver.txt")
...
Global $7zipwin = GUICreate("ECU Anti-Virus Setup", 250, 50, -1, -1, BitXOR($GUI_SS_DEFAULT_GUI, $WS_MINIMIZEBOX))
...
If $NeedSysRestore = "True" Then
   ...
   Local $CrtSysRes = Run("PowerShell.exe -ExecutionPolicy Bypass -Command ""Checkpoint-Computer -Description 'ECU Anti-Virus Setup'""", "")
...
If $NeedExtract = "True" Then
   RunWait(@TempDir & '\files.exe -o"' & @TempDir & '/SAVInstall/" -y', "", @SW_HIDE)
...
Global $mainwin = GUICreate("ECU Anti-Virus Setup", 450, 400, -1, -1, BitXOR($GUI_SS_DEFAULT_GUI, $WS_MINIMIZEBOX))
...
Func stageOne() ;Creates the base UI, nothing special about it except contact info and instructions.
   ...
   Global $nameText = GUICtrlCreateLabel("ECU Anti-Virus Setup (BETA)", 0, 10, 450, 20, $SS_CENTER)
   ...
   Global $bodyText = GUICtrlCreateLabel("Welcome to the ECU Anti-Virus Setup tool, please click next to start the installation process." & @CRLF & "" & @CRLF & "If you have any issues or need help, please visit Pirate Techs in Rawl 108 or call us at (252) 328-5407." & @CRLF & "" & @CRLF & "Version: " & $sver & "(" & $sdate & ")", 20, 70, 410, 270)
...
Func stageFour()
   ...
   If $SAVhadError = True Then
	  GUICtrlSetData($bodyText, GUICtrlRead($bodyText) & "" & @CRLF & "Some installs had errors. Please consult the logs in the " & @HomeDrive & "\SAVLogs\ folder or visit PirateTechs in Rawl 108." )
   Else
...
Func openTempFolder()
   Run("explorer.exe """ & @TempDir & "\SAVInstall\files""")
   Call("closedown")
EndFunc
